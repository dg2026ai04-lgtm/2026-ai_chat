...
.grade-system-banner-9 {{
    ...
    border: 2px solid #6d28d9; border-radius: 14px;
    padding: 1rem 1.2rem; margin: 0.7rem 0; text-align: center;
}}
...
.grade-table th {{
    ...
    padding: 0.5rem 0.7rem; text-align: center; border: 1px solid {T['BORDER']};
}}
.grade-table td {{
...
with st.sidebar:
    ...
    st.caption(f"현재 {user_age}세 -> {grade_system} 적용")
...
def encode_image(f):
    ...
    mime = (
        f.type if f.type in ["image/jpeg", "image/png", "image/gif", "image/webp"]
        else "image/png"
    )
    return b64, mime
...
def build_system_prompt(extra=""):
    career_ctx = (
        ...
        f"나이: {user_age}세\n"
        f"내신 등급제: {grade_system}\n\n"
        f"답변 시 진로와 학과를 자연스럽게 연결해주세요. "
        f"무관한 질문이면 억지 연결은 불필요합니다."
    )
    ...
    if app_mode == "💬 일반 모드":
        ...
        if learning_feature == "💬 질문하기":
            ...
            role = (
                ...
                f"퀴즈 출제자로서 '문제 내줘'라고 하면 {dm[quiz_difficulty]} "
                f"문제를 1개 출제하세요."
            )
        else:
...
def show_grade_calculator():
    ...
    if st.session_state.total_input_tokens > 0:
        ...
        st.markdown(
            ...
            f'누적 비용: ${cost:.4f} (약 {cost * 1400:.0f}원) | '
...
st.markdown(
    f'<div class="mode-bar">{mt} | {grade_system}</div>',
    unsafe_allow_html=True
)
st.markdown('<div class="divider"></div>', unsafe_allow_html=True)
...
if app_mode == "🎓 고교학점제 모드":
    ...
    st.markdown('<h3>나의 진로 설정</h3>', unsafe_allow_html=True)
    st.markdown(
        ...
        f'<p style="color:{T["TEXT2"]};font-size:0.82rem;'
        f'margin-top:0.7rem;margin-bottom:0;">'
        f'아래 채팅에서 맞춤형 AI 답변을 받을 수 있습니다!</p>'
        f'</div>',
        unsafe_allow_html=True
    )
    ...
    st.markdown(
        ...
        f'{fg.get(hschool_feature, "질문을 입력해주세요!")}'
        f'</span></div>',
        unsafe_allow_html=True
    )
    st.markdown('<div class="divider"></div>', unsafe_allow_html=True)
    ...
    for msg in st.session_state.chat_history:
        if msg["role"] == "user":
            ...
            st.markdown(
                ...
                f'<div class="chat-role chat-role-user">나</div>{dc}</div>',
                unsafe_allow_html=True
            )
        else:
            st.markdown(
...
if app_mode == "🎓 고교학점제 모드":
    ...
    card_title = hschool_feature
    fc = {
        "📘 추천 과목 안내": (
            f"{career_field} 진로와 {final_dept} 입시에 맞는 과목을 추천하고 이유를 설명해주세요."
        ),
        "📝 생기부 주제 추천": (
            f"{career_field}와 {final_dept} 관련 생기부 탐구 주제를 여러 개 제안해주세요."
        ),
        "🏃 활동 추천": (
            f"{career_field} 진로와 {final_dept} 입시에 도움되는 활동을 추천해주세요."
        ),
    }
...
if st.button("AI에게 질문하기", type="primary", use_container_width=True):
    if not user_question.strip() and not uploaded_image:
        st.warning("질문을 입력하거나 이미지를 업로드해주세요!")
    else:
...
if st.session_state.total_input_tokens > 0:
    st.markdown('<div class="divider"></div>', unsafe_allow_html=True)
    st.markdown(
        '<div class="card"><h3>📊 누적 사용량 & 비용</h3>',
        unsafe_allow_html=True
    )
